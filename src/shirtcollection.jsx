...
function Shirtcollection() {
  ...
  const slist = [
    { id: "plain", type: "PLAIN SHIRT", image: pshirt, path: "/plainshirt" },
    { id: "checked", type: "CHECKED SHIRT", image: cshirt, path: "/checkedshirt" },
    { id: "printed", type: "PRINTED SHIRT", image: prshirt, path: "/printedshirt" },
    { id: "stripes", type: "STRIPES SHIRT", image: stshirt, path: "/stripesshirt" },
  ];

  const Shirtlist = slist.map((types) =>
    <Home type={types.type} key={types.id} image={types.image} path={types.path} shopId={shopId} />
  );
